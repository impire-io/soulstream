// Command soulstream-node serves the remote MCP door for one realm: a URL a
...
func main() {
	fs := flag.NewFlagSet("soulstream-node", flag.ExitOnError)
	...
	fmt.Fprintf(os.Stderr, "soulstream-node %s: realm %q on %s, %s, listening on %s\n",
