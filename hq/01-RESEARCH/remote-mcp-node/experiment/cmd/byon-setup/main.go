// Command byon-setup configures auth callout on the Synadia Cloud BYON via
// the Cloud API — the Bar 4 §0a acts, scripted: the control (AUTH) account,
// the programmatic scoped signing keys (their seeds are shown exactly once —
// this tool is that once, writing them 0600 for the vault import), the
// callout wiring, and the issuer user's creds. Experiment code, not product.
//
// Usage:
//
	...
	return &syncp.UserPermissionLimits{
		Permissions: syncp.Permissions{
			Pub: &syncp.Permission{Allow: []string{
				...
				"soulidentity.{{account-subject()}}.{{name()}}.keys.public",
...
func main() {
	...
	fmt.Printf(`
byon-setup: done. Next (any tailnet machine; xkey seeds via soulidentity keygen):

  soulidentity serve --context impire-dev-platform \
    --callout-creds %s --auth-account %s --callout-ttl 5m

  soulidentity key import --context impire-dev-platform --as %s/<your-user> \
  ...
  soulidentity token create --context impire-dev-platform --as %s/<your-user> \
    --account %s --user daan --label "claude desktop"
  soulidentity sentinel --context impire-dev-platform --as %s/<your-user> > sentinel.creds
`,
		credsPath, ctrlPub,
