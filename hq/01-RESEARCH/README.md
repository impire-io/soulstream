...
shape for the protocol?", "does this NATS mechanism actually behave the way we
think?" — the ones spec-kit can't help with because spec-kit assumes the answer
is already known.
...
- **States are exactly** `active`, `graduated`, `abandoned` — and no folder with
  a terminal state lingers here (the structural lint enforces both).
...
- **Bars before experiments.** The pass/fail criteria are written down before
  any run; if a bar proves degenerate it is amended openly with the raw numbers
  recorded (constitution — Honest Measurement lives in the working agreement's
  `[measured]` discipline).
