# HQ — Soulstream's headquarters

Everything about *how this project is run* lives here. The library code lives in
the package directories (`record/`, `identity/`, `realm/`, `topic/`, `registry/`,
`curator/`, `cmd/`, `internal/`); plain-words explanations in `docs/`; frozen
per-feature spec-kit artifacts in `specs/NNN-*/`. Everything else — why
Soulstream exists, what we're investigating, what we've designed, what we're
building, and what happened — lives in one of the areas below.

| Area | What it holds | When you touch it |
|---|---|---|
| [`00-GENESIS/`](00-GENESIS/README.md) | Vision, constitution, working agreement, rationale | When deciding *whether* / *how* to do something |
| [`01-RESEARCH/`](01-RESEARCH/README.md) | Active research topics (one folder each) | While investigating an open question |
| [`02-DESIGN/`](02-DESIGN/README.md) | The normative design: `core/` (the protocol) + `extensions/` (optional conventions) | When research graduates, or a build changes behavior |
| [`03-IMPLEMENTATION/`](03-IMPLEMENTATION/README.md) | The roadmap: what to build, in what order, behind which gate | When planning or landing a feature |
...
`99-ARCHIVE/` holds superseded material (the pre-restructure design set), kept
for provenance with its content frozen — it is history, not the live design.
