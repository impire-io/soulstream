...
- [x] T001 Add `TypeCommentReply`/`TypeCommentResolve`/`TypeEdit`/
...
- [x] T002 [US1] Fold in `topic/view.go`: `EditStamp`,
...
- [x] T003 [P] [US1] Server-free fold tests in `topic/upkeep_fold_test.go`:
...
- [x] T004 [US1] Baking: `cleanBakedContributions` deep-copies `Edits` and zeroes
...
- [x] T005 [US1] Write side in `topic/upkeep.go`: `Handle.Reply` (mentions),
...
- [x] T006 [US1] CLI `reply`/`edit`/`resolve` in
...
- [x] T007 [US1] MCP `soulstream_reply_comment` / `soulstream_resolve_comment` /
...
- [x] T008 [US1] ELI5 docs: new `docs/editing.md` (pencil edits & margin notes —
...
- [x] T009 [US2] Fold + derivation: `Attachment.{Removed,RemovedBy}` and the
...
- [x] T010 [US2] Write side + GC: `Handle.RemoveAttachment` in `topic/upkeep.go`;
...
- [x] T011 [US2] Clients + docs: CLI `detach` + removed markers in listings
...
- [x] T012 [US3] Lifecycle: `definedLifecycle` accepts `Dormant`
...
- [x] T013 [P] [US3] Pure rules in `topic/upkeep.go`: `DormantEligible`
...
- [x] T014 [US3] Sweeps: `curator.Options.{MarkDormant,ReclaimAfter}` +
...
- [x] T015 [US3] Docs: `docs/lifecycle.md` (dormant = napping, any content op
...
- [x] T016 Walk quickstart.md against the real CLI and reconcile drift; sweep
