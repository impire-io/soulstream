...
- [X] T001 `go get github.com/modelcontextprotocol/go-sdk@v1.6.1`; confirm it resolves and `go build ./...` still passes.

## Phase 2: Foundational (Blocking)

- [X] T002 Extend `topic/notify.go`: `FetchInbox(ctx, c, persona, limit) ([]Notification, error)` —
...
- [X] T003 Extend `topic/notify_test.go`: post N mentions to a persona; `FetchInbox` returns them
  newest-first, honours the limit, and returns empty (no error) when there are none. (SC-003)
- [X] T004 `internal/mcpserver/server.go`: `newHandlers(c)`; `NewServer(c) *mcp.Server` (registers the
  8 tools); result helpers (`jsonResult`, `textResult`). (FR-001/013)
- [X] T005 `cmd/soulstream-mcp/main.go`: resolve config (env + flags), require a persona,
...
- [X] T006 [US1] `internal/mcpserver/tools.go`: `board` (no input → JSON board) and `showTopic`
  (`{path}` → JSON view; empty/malformed reported). (FR-005/006)
- [X] T007 [US1] Test `internal/mcpserver/server_test.go`: `board` returns started topics; `showTopic`
  returns a materialised view with contributions; a bad path is reported (tool error/empty), no panic.
  (SC-001/004)
- [X] T008 [P] [US1] ELI5 doc `docs/mcp.md`: "the same door, for agents", with the tool list + config.
  (Constitution III)
...
- [X] T009 [US2] Extend `tools.go`: `startTopic` (`{name,subject?,tags?,parent?}` → path), `postTurn`
  (`{path,body}` → op-id; mentions via library), `addComment` (`{path,anchor_op_id,body}` → op-id);
  each materialises first. (FR-007/008/009)
- [X] T010 [US2] Extend `server_test.go`: `startTopic → postTurn (with @mention) → addComment`; a
...
- [X] T011 [US3] Extend `tools.go`: `checkInbox` (`{limit?}` → JSON notifications, newest-first). (FR-012)
- [X] T012 [US3] Extend `server_test.go`: another persona mentions this one; `checkInbox` returns the
...
- [X] T013 [US4] Extend `tools.go`: `attachText` (`{path,name,content_type?,body}` → object key) and
  `closeTopic` (`{path}` → closed). (FR-010/011)
- [X] T014 [US4] Extend `server_test.go`: `attachText` stores + lists an attachment (object key
...
- [X] T015 [P] Update `README.md`: a `cmd/soulstream-mcp` line + the 8 tools; link the doc.
- [X] T016 Run `go mod tidy`; `go vet ./...` clean; confirm `go build ./cmd/soulstream-mcp` builds.
