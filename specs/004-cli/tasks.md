...
- [ ] T001 `cmd/soulstream/main.go`: `func main(){ os.Exit(cli.Main(os.Args[1:])) }`.

## Phase 2: Foundational (Blocking)

- [ ] T002 `internal/cli/cli.go`: `Config`; `Run(ctx, args, stdout, stderr, connect) int` (global
...
- [ ] T003 `internal/cli/connect.go`: `Connector` type; `realmConnect` (via `realm.Connect`);
...
- [ ] T004 [P] `internal/cli/render.go`: text + JSON renderers for a provision report, the board,
...
- [ ] T005 [US1] `internal/cli/commands.go`: `cmdProvision` (print per-artefact result) and
  `cmdBoard` (`--json`; text list of path/name/lifecycle; empty realm → empty). (FR-004/005)
- [ ] T006 [US1] Test `internal/cli/cli_test.go`: via `Run` + injected embedded connector —
  ...
  + stderr. (SC-001/002)
- [ ] T007 [P] [US1] ELI5 doc `docs/cli.md` (start): the CLI as "the remote control — one word per
...
- [ ] T008 [US2] Extend `commands.go`: `cmdStart` (`--subject/--tag/--parent`, print path),
  `cmdShow` (`--json`; text of baseline/contributions+mentions/attachments/lifecycle), `cmdPost`,
  `cmdComment`. (FR-006/007/008/009)
- [ ] T009 [US2] Extend `cli_test.go`: `start → post (with @mention) → comment → show` prints the
...
- [ ] T010 [US3] `internal/cli/stream.go`: `cmdWatch <path>` (`topic.Follow`, print each new
...
- [ ] T011 [US3] Test `internal/cli/stream_test.go`: with a short timeout ctx, `watch` prints a turn
...
- [ ] T012 [US4] Extend `commands.go`: `cmdAttach` (`--type/--anchor`, print object key), `cmdGet`
  (`--force`; write bytes, verify digest, no clobber), `cmdClose`. (FR-010/011/012)
- [ ] T013 [US4] Extend `cli_test.go`: `attach → show` lists it; `get` reproduces the bytes and
...
- [ ] T014 [P] Finish `docs/cli.md`; update `README.md` (a `cmd/soulstream` line + build note).
- [ ] T015 Run `go mod tidy`; `go vet ./...` clean; confirm `go build ./cmd/soulstream` produces a binary.
- [ ] T016 Final gate: `make check` green — all tests pass (none skipped), lint 0. (SC-005)
