...
- [x] T001 Create `node/` module skeleton: `node/go.mod` (module
...
- [x] T004 Promote `internal/mcpserver` → public `mcpserver/`: `git mv`,
...
- [x] T005 Add options to `mcpserver.NewServer(c *realm.Client, opts
...
- [x] T006 Build the node test rig `node/rigtest/rig.go`: embedded
...
- [x] T007 Node config: `node/config.go` — `Config` struct + validation
...
- [x] T008 [US1] `node/principal.go`: derive `(account, persona)` from
...
- [x] T009 [US1] `node/pool.go` build path: pool map + entry lifecycle
...
- [x] T010 [US1] `node/node.go`: `New(cfg)`, `getServer(r)` factory
...
- [x] T011 [US1] `node/oauth.go`: RFC 9728 metadata route + 401
...
- [x] T012 [US1] `node/cmd/soulstream-node/main.go`: flags/env mirroring
      Config, `-version`, startup teaching checks (realm stream exists —
      read-only, R9; never provisions), serve `Handler()`
- [x] T013 [US1] Integration test `node/node_test.go`: the Independent
...
- [x] T014 [US1] No/invalid bearer tests: public mode → 401 + challenge
      steering (scenario 3); local mode → 400; nothing reaches the realm
- [x] T015 [P] [US1] Docs: create `docs/remote-node.md` ELI5 core ("a
...
- [x] T016 [US2] Session binding in `node/pool.go` + `node/node.go`: bind
...
- [x] T017 [US2] Candidate probe in `node/pool.go`: unbound request with a
...
- [x] T021 [US2] Custody audit test in `node/custody_test.go`: run a
...
- [x] T022 [US2] Token-material log audit in `node/logaudit_test.go`:
...
- [x] T024 [US3] Refresh test in `node/refresh_test.go`: TTL ~2s, client
...
- [x] T025 [US3] Revocation tests in `node/revoke_test.go`: delete token
...
- [x] T028 [US4] AS stand-in `node/rigtest/asstub.go` written FROM
...
- [x] T029 [US4] Full-flow test `node/oidc_flow_test.go`: scripted client
      ...
      restarts (contract §3)
- [x] T030 [US4] Conformance-refusal tests in `node/oidc_flow_test.go`:
...
- [x] T032 [US5] Proxy-fronted test in `node/node_test.go`: requests with
...
- [x] T037 Cycle-guard + custody asserts: test that core `go.mod` gained
...
- [x] T040 Full gate: `make check` green (both modules, all tests, none
