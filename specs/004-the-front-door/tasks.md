...
- [ ] T001 `go.mod`: soulstream → v0.7.0; add `soulstream/node` at its
      pseudo-version; tidy (archivist-against-v0.7.0 compatibility rides
      the gate).

## Phase 2: Foundational (ceremony/config)

- [ ] T002 `ceremony/`: `planes.door {enabled, listen}` per
...
- [ ] T003 [US1] `node/node.go`: `startDoor` — pre-flight bind (held
      ...
      teardown.
- [ ] T004 [US1] `cmd/soulnode/main.go`: init's founding block and up's
      serving lines name the door URL when enabled.
- [ ] T005 [US1] `node/node_test.go` `TestFrontDoor`: MCP client
...
- [ ] T006 Full gate green (SC-004).
- [ ] T007 Landing duties: journey 0006; roadmap Phase 2 (local mode
