...
- [ ] T022 [P] [US3] `record/id.go`: `NewID() string` → `uuid.NewString()` (lowercase 8-4-4-4-12); unit test `record/id_test.go` asserting format + uniqueness across N calls. (FR-013)
- [ ] T023 [P] [US3] `record/errors.go`: `FieldError{Field,Reason}` + sentinels `ErrMissingField`, `ErrBadVersion`, `ErrBadTimestamp`, `ErrBadAuthor`, `ErrBadID` (errors.Is-compatible). (FR-016)
- [ ] T024 [US3] `record/record.go`: `Record` struct (ID, Author, Parents, Type, Timestamp, Signature, Payload, Extras); `const Version = 1`; `Validate()`; `Build() (headers, payload, err)` writing `Nats-Msg-Id`, `Soulstream-Version/Author/Parents/Type/Ts/Sig` (+Extras), with the **absent⇔empty parents** rule; `Parse(headers, payload) (Record, error)` enforcing required fields, `Version==1`, RFC3339 ts, well-formed author (via `identity.CheckName`), preserving unknown `Soulstream-*` headers into `Extras`. (FR-010/011/012/014/015/016/017/018)
