...
// repoRoot locates the soulnode repository root relative to this source file
...
// TestEpisodesRecordReversalCondition holds every episode to the rule in
// hq/04-JOURNEY/TEMPLATE.md: the Reversal condition line is required on all
// episodes from 0001 onward — soulnode never had a pre-split exemption.
...
func TestConstitutionSymlinkResolvesToGenesis(t *testing.T) {
	...
	if !strings.Contains(string(data), "# SoulNode Constitution") {
		t.Error("constitution missing the expected '# SoulNode Constitution' heading")
	}
}
