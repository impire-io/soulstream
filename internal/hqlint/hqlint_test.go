...
// (a dangling link would silently fork the constitution via spec-kit's template
// re-copy), and relative markdown links inside hq/ resolve. The frozen
// 99-ARCHIVE subtree is permitted as an extra area and is excluded from the
// link check — it is superseded material, not the live design.
...
func TestConstitutionSymlinkResolvesToGenesis(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("dangling symlink — spec-kit would re-copy its template over it: %v", err)
	}
	resolvedCanonical, err := filepath.EvalSymlinks(canonical)
	if err != nil {
		t.Fatalf("canonical constitution missing: %v", err)
	}
	if resolvedLink != resolvedCanonical {
		t.Fatalf("symlink resolves to %s, not %s", resolvedLink, resolvedCanonical)
	}
	data, err := os.ReadFile(canonical)
	if err != nil {
		t.Fatalf("reading canonical constitution: %v", err)
	}
	if !strings.Contains(string(data), "# Soulstream Constitution") {
		t.Error("canonical constitution missing its '# Soulstream Constitution' heading")
	}
}
