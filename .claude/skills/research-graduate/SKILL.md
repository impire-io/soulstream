...
4. **Route the outcome.**
   - `design`: create or update the design doc under `hq/02-DESIGN/` — `core/` if
     it is protocol-normative, `extensions/` if it is an optional convention —
     functional level, explicit enough for `/speckit-specify`. The episode links
     it.
   - `artifact`: the deliverable ships wherever it belongs (an example, a `docs/`
     page, a tool); the episode links it.
...
7. **Gate, commit (never push).** Run the full quality gate
   (`make fmt && make test && make lint`) — the structural lint (`internal/hqlint`,
   under `make test`) must be green. Stage only the touched paths by explicit
